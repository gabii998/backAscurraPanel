...
export class CreateIgExamplePost {
  ...
  async execute(input: CreateIgExamplePostInput): Promise<IgExamplePost> {
    if (!input.file || !ALLOWED_TYPES.has(input.file.mimetype)) throw new Error("MISSING_FIELDS");
    ...
    try {
      ...
      if (input.assetType !== "style_reference") return prisma.igExamplePost.update({ where: { id: example.id }, data: { imageUrl, objectKey, fileName: input.file.originalname, mimeType: input.file.mimetype, fileSize: input.file.size, summaryStatus: "done" } });
